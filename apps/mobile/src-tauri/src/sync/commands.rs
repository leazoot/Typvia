...
//!
//! Wire names, argument names and DTO shapes are the desktop host's, so the
//! shared typed IPC client drives both hosts unchanged.
//!
//! Unlike the desktop host every command is `pub async fn` and hops to the
//! blocking pool through `blocking_state`. Tauri runs synchronous commands on
//! the main thread, and a sync round is exactly what must never run there: it
//! does network IO and SQLite work, and on Android it reaches the SecureStore
//! JNI bridge, whose Kotlin side posts to the main looper and parks the
//! calling thread on the result — on the main thread that deadlocks, and even
//! without it a round's latency would freeze the WebView and risk an ANR.
//! `State` borrows the handle, so the states are resolved inside the pool
...
use super::{SyncHost, map_sync, mark_recovery_exported, recovery_exported_at, setup_state};
use crate::commands::{AppState, blocking_state, now_ms};

/// Current sync state for the Sync page and the Settings group.
#[tauri::command]
pub async fn sync_status(app: tauri::AppHandle) -> Result<SyncStatusDto, IpcError> {
    blocking_state(app, |state, sync| {
        let host = sync.lock()?;
        let conn = state.lock()?;
...
pub async fn sync_enable(
    ...
    blocking_state(app, move |state, sync| {
        let now = now_ms()?;
...
/// mirror of the desktop command in the mobile async shape.
#[tauri::command]
pub async fn sync_enable_webdav(
    app: tauri::AppHandle,
    base_url: String,
...
#[tauri::command]
pub async fn pairing_begin_webdav(
    app: tauri::AppHandle,
    base_url: String,
    credentials: Option<String>,
...
#[tauri::command]
pub async fn recovery_recover_webdav(
    app: tauri::AppHandle,
    base_url: String,
    credentials: Option<String>,
    code: String,
...
#[tauri::command]
pub async fn sync_disable(app: tauri::AppHandle) -> Result<SyncStatusDto, IpcError> {
    blocking_state(app, |state, sync| {
        let now = now_ms()?;
        let mut host = sync.lock()?;
        let conn = state.lock()?;
...
#[tauri::command]
pub async fn sync_resume(app: tauri::AppHandle) -> Result<SyncStatusDto, IpcError> {
    blocking_state(app, |state, sync| {
        let now = now_ms()?;
        let mut host = sync.lock()?;
        let conn = state.lock()?;
...
#[tauri::command]
pub async fn sync_now(app: tauri::AppHandle) -> Result<SyncRoundDto, IpcError> {
    blocking_state(app, |state, sync| {
        let now = now_ms()?;
        let mut host = sync.lock()?;
        run_round(&state.conn_handle(), state.secure_store(), &mut host, now)
    })
    .await
}
...
#[tauri::command]
pub async fn sync_devices(app: tauri::AppHandle) -> Result<Vec<SyncDeviceDto>, IpcError> {
    blocking_state(app, |state, sync| {
        let mut host = sync.lock()?;
        let conn = state.lock()?;
        let devices = host
...
#[tauri::command]
pub async fn sync_revoke_device(app: tauri::AppHandle, device_id: String) -> Result<u32, IpcError> {
    blocking_state(app, move |state, sync| {
        let now = now_ms()?;
        let mut host = sync.lock()?;
        let conn = state.lock()?;
...
#[tauri::command]
pub async fn sync_rotate_key(app: tauri::AppHandle) -> Result<u32, IpcError> {
    blocking_state(app, |state, sync| {
        let now = now_ms()?;
        let mut host = sync.lock()?;
        let conn = state.lock()?;
...
#[tauri::command]
pub async fn pairing_poll(app: tauri::AppHandle) -> Result<Option<PairingClaimDto>, IpcError> {
    blocking_state(app, |_state, sync| {
        let mut host = sync.lock()?;
...
#[tauri::command]
pub async fn pairing_cancel(app: tauri::AppHandle) -> Result<(), IpcError> {
    blocking_state(app, |_state, sync| {
        let mut host = sync.lock()?;
        host.pairing = None;
        host.claimed = None;
...
/// anything yet.
#[tauri::command]
pub async fn pairing_sas(app: tauri::AppHandle, code: String) -> Result<PairingSasDto, IpcError> {
    blocking_state(app, move |state, sync| {
        let host = sync.lock()?;
        let conn = state.lock()?;
...
#[tauri::command]
pub async fn recovery_publish(app: tauri::AppHandle) -> Result<RecoveryCodeDto, IpcError> {
    blocking_state(app, |state, sync| {
        let now = now_ms()?;
        let mut host = sync.lock()?;
        let conn = state.lock()?;
...
#[tauri::command]
pub async fn sync_conflicts(app: tauri::AppHandle) -> Result<Vec<ConflictPairDto>, IpcError> {
    blocking_state(app, |state: &AppState, _sync| {
        let now = now_ms()?;
        let conn = state.lock()?;
...
pub async fn sync_conflict_resolve(
    ...
    blocking_state(app, move |state: &AppState, _sync| {
        let now = now_ms()?;
        let conn = state.lock()?;
