...
//! Sync host assembly: builds the engine, keeps the core write-path observer
//! registered, and maps engine failures onto the stable error codes.
//!
//! The engine lives here rather than in `host-service` because the shared
//! layer must not depend on `crates/sync`. Everything below is wiring:
//! sessions, ordering and error translation. The protocol itself — sealing,
//! trust, pairing, merging — stays in `crates/sync`.
//!
//! Lock order across the whole host is `sync` → `conn` → `vault`. Every call
//! takes its guards in that order, so the three mutexes cannot deadlock
//! against each other.
...
use typvia_sync::{
    ...
    PairingHandle, RecoveryError, SyncEngine, SyncError, SyncFailure, SyncKeys, TransportError,
    WebdavClient, WebdavCredentials, WebdavStore, ensure_local_device,
};

/// The persisted configuration, read straight from `sync_config`.
pub(crate) struct SyncSetupState {
...
pub struct SyncHost {
    ...
    pub(crate) engine: Option<SyncEngine>,
    /// Kept alive for as long as the write path should seal into the outbox.
    observer: Option<ObserverGuard>,
    pub(crate) pairing: Option<PairingHandle>,
    pub(crate) claimed: Option<ClaimedPairing>,
    pub(crate) last_sync_at: Option<TimestampMs>,
    /// Why the last round stopped, when it stopped. Cleared by the next round
    /// that finishes — a reason that outlives the problem is a reason that
    /// makes the screen lie.
    pub(crate) last_failure: Option<SyncFailure>,
...
impl SyncHost {
    ...
    pub(crate) fn run_engine_round(
        ...
        let Some(engine) = self.engine.as_mut() else {
            self.last_failure = Some(SyncFailure::NotSetUp);
            return Err(IpcError::conflict("sync is not set up on this device"));
        };
        let outcome = match &self.webdav {
            Some(dav) => engine.sync_webdav(db, store, dav, now),
            None => engine.sync(db, store, now),
        };
        // Recorded here rather than at the call sites: this is the one place
        // that still holds the engine's own error, and every round comes
        // through it.
        match outcome {
            Ok(report) => {
    ...
    pub(crate) fn detach_sealer(&mut self) {
        self.observer = None;
    }
    ...
    pub(crate) fn engine_mut(&mut self) -> Result<&mut SyncEngine, IpcError> {
        self.engine
            .as_mut()
            .ok_or_else(|| IpcError::conflict("sync is not set up on this device"))
    }
...
pub(crate) fn setup_state(conn: &Connection) -> Result<SyncSetupState, IpcError> {
...
pub(crate) fn recovery_exported_at(data_dir: &Path) -> Option<TimestampMs> {
...
pub(crate) fn mark_recovery_exported(data_dir: &Path, now: TimestampMs) {
...
pub(crate) fn map_transport(error: TransportError) -> IpcError {
    match error {
